 ...
 * "Powered by SugarCRM".
 ********************************************************************************/
function Datetimecombo(datetime,field,timeformat,tabindex,showCheckbox,checked,allowEmptyHM,minutesStep){this.minutesStep=minutesStep;this.datetime=datetime;this.allowEmptyHM=allowEmptyHM;if(typeof this.datetime=="undefined"||datetime==''||trim(datetime).length<10){this.datetime='';var d=new Date();var month=d.getMonth();var date=d.getDate();var year=d.getYear();var hours=d.getHours();var minutes=d.getMinutes();}
this.fieldname=field;if(datetime!='')
{parts=datetime.split(' ');this.hrs=parseInt(parts[1].substring(0,2),10);this.mins=parseInt(parts[1].substring(3,5),10);if(this.mins%5!=0){this.mins=Math.ceil(this.mins / 5)*5;}}
...
this.meridiem=this.hasMeridiem?trim(this.datetime.substring(16)):'';this.datetime=this.datetime.substr(0,10);this.showCheckbox=showCheckbox;this.checked=parseInt(checked);YAHOO.util.Selector.query('input#'+this.fieldname+'_date')[0].value=this.datetime;if(this.mins<0||this.mins>59){this.hrs+=1;this.mins=0;if(this.hasMeridiem&&this.hrs==12){if(this.meridiem=="pm"||this.meridiem=="am"){if(this.meridiem=="pm"){this.meridiem="am";}else{this.meridiem="pm";}}else{if(this.meridiem=="PM"){this.meridiem="AM";}else{this.meridiem="PM";}}}
...
var minutes_step=parseInt(this.minutesStep);var step=!isNaN(minutes_step)?minutes_step:15;for(var minute=0;minute<=59;minute+=step){var parsed_number=(minute).toLocaleString(undefined,{minimumIntegerDigits:2,useGrouping:false});text+='\n<option value="'+parsed_number+'" '+(this.mins==minute?"SELECTED":"")+'>'+parsed_number+'</option>';}
...
YAHOO.util.Selector.query('input#'+this.fieldname)[YAHOO.util.Selector.query('input#'+this.fieldname).length-1].value=newdate;if(updateListeners)
...
{YAHOO.util.Selector.query('input#'+flag)[0].value=1;YAHOO.util.Selector.query('input#'+this.fieldname)[0].value='';YAHOO.util.Selector.query('input#'+date)[0].disabled=true;YAHOO.util.Selector.query('select#'+hours)[0].disabled=true;YAHOO.util.Selector.query('select#'+mins)[0].disabled=true;}else
