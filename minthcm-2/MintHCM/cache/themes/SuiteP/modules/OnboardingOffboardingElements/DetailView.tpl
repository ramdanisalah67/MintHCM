...
<div class="buttons">
{if $bean->aclAccess("edit")}<input title="{$APP.LBL_EDIT_BUTTON_TITLE}" accessKey="{$APP.LBL_EDIT_BUTTON_KEY}" class="button primary" onclick="var _form = document.getElementById('formDetailView'); _form.return_module.value='OnboardingOffboardingElements'; _form.return_action.value='DetailView'; _form.return_id.value='{$id}'; _form.action.value='EditView';SUGAR.ajaxUI.submitForm(_form);" type="button" name="Edit" id="edit_button" value="{$APP.LBL_EDIT_BUTTON_LABEL}">{/if} 
{if $bean->aclAccess("edit")}<input title="{$APP.LBL_DUPLICATE_BUTTON_TITLE}" accessKey="{$APP.LBL_DUPLICATE_BUTTON_KEY}" class="button" onclick="var _form = document.getElementById('formDetailView'); _form.return_module.value='OnboardingOffboardingElements'; _form.return_action.value='DetailView'; _form.isDuplicate.value=true; _form.action.value='EditView'; _form.return_id.value='{$id}';SUGAR.ajaxUI.submitForm(_form);" type="button" name="Duplicate" value="{$APP.LBL_DUPLICATE_BUTTON_LABEL}" id="duplicate_button">{/if} 
...
{sugar_translate label='LBL_PANEL_ASSIGNMENT' module='OnboardingOffboardingElements'}
</a>
</li>
</ul>
</li>
...
<li>{if $bean->aclAccess("detail")}{if !empty($fields.id.value) && $isAuditEnabled}<input id="btn_view_change_log" title="{$APP.LNK_VIEW_CHANGE_LOG}" class="button" onclick='open_popup("Audit", "600", "400", "&record={$fields.id.value}&module_name=OnboardingOffboardingElements", true, false,  {ldelim} "call_back_function":"set_return","form_name":"EditView","field_to_name_array":[] {rdelim} ); return false;' type="button" value="{$APP.LNK_VIEW_CHANGE_LOG}">{/if}{/if}</li>
</ul>        </li>
...
{if $fields.kind_of_element.acl > 0}
...
{if !$fields.kind_of_element.hidden}
{counter name="panelFieldCount" print=false}


{if is_string($fields.kind_of_element.options)}
<input type="hidden" class="sugar_field" id="{$fields.kind_of_element.name}" value="{ $fields.kind_of_element.options }">
{ $fields.kind_of_element.options }
{else}
<input type="hidden" class="sugar_field" id="{$fields.kind_of_element.name}" value="{ $fields.kind_of_element.value }">
{ $fields.kind_of_element.options[$fields.kind_of_element.value]}
{/if}
{/if}
...
{if $fields.days_from_start.acl > 0}
...
{if !$fields.days_from_start.hidden}
{counter name="panelFieldCount" print=false}

<span class="sugar_field" id="{$fields.days_from_start.name}">
{sugar_number_format precision=0 var=$fields.days_from_start.value}
</span>
{/if}
...
{if $fields.task_duration_hours.acl > 0}
...
{if !$fields.task_duration_hours.hidden}
{counter name="panelFieldCount" print=false}
<span id="task_duration_hours" class="sugar_field">{$fields.task_duration_hours.value}{$MOD.LBL_HOURS_ABBREV} {$fields.task_duration_minutes.value}{$MOD.LBL_MINSS_ABBREV}&nbsp;</span>
{/if}
...
{if $fields.securitygroup_unit_name.acl > 0}
...
{if !$fields.securitygroup_unit_name.hidden}
{counter name="panelFieldCount" print=false}

{if !empty($fields.securitygroup_unit_id.value)}
{capture assign="detail_url"}index.php?module=SecurityGroups&action=DetailView&record={$fields.securitygroup_unit_id.value}{/capture}
<a href="{sugar_ajax_url url=$detail_url}">{/if}
<span id="securitygroup_unit_id" class="sugar_field" data-id-value="{$fields.securitygroup_unit_id.value}">{$fields.securitygroup_unit_name.value}</span>
{if !empty($fields.securitygroup_unit_id.value)}</a>{/if}
{/if}
...
<span id="user_id" class="sugar_field" data-id-value="{$fields.user_id.value}">{$fields.user_name.value}</span>
{/if}
...
{capture name="label" assign="label"}{sugar_translate label='LBL_DATE_ENTERED' module='OnboardingOffboardingElements'}{/capture}
{$label|strip_semicolon}:
</div>
...
{capture name="label" assign="label"}{sugar_translate label='LBL_DATE_MODIFIED' module='OnboardingOffboardingElements'}{/capture}
{$label|strip_semicolon}:
</div>
