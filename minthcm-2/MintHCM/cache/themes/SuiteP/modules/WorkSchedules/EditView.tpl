...
{/if}
{if $bean->aclAccess("detail")}{if !empty($fields.id.value) && $isAuditEnabled}<input id="btn_view_change_log" title="{$APP.LNK_VIEW_CHANGE_LOG}" class="button" onclick='open_popup("Audit", "600", "400", "&record={$fields.id.value}&module_name=WorkSchedules", true, false,  {ldelim} "call_back_function":"set_return","form_name":"EditView","field_to_name_array":[] {rdelim} ); return false;' type="button" value="{$APP.LNK_VIEW_CHANGE_LOG}">{/if}{/if}
</div>
{/if}
...
{sugar_translate label='DEFAULT' module='WorkSchedules'}
</div>
</a>
</div>
...
{minify}
{capture name="label" assign="label"}{sugar_translate label='LBL_ASSIGNED_TO_NAME' module='WorkSchedules'}{/capture}
...
data-validation="{literal}AEM(isSuperior($assigned_user_id),'LBL_WRONG_ASSIGNED_USER'){/literal}" >
...
{minify}
{capture name="label" assign="label"}{sugar_translate label='LBL_TYPE' module='WorkSchedules'}{/capture}
{$label|strip_semicolon}:
...
id="{$fields.type.name}" 
title=''               
data-validation="{literal}AEM(callCustomApi(WorkSchedules,canChangeTypeToWorkOff,{id:$id,type:$type}),'LBL_ERR_CANT_CHANGE_TYPE_TO_WORK_OFF'){/literal}" >
...
{minify}
{capture name="label" assign="label"}{sugar_translate label='LBL_STATUS' module='WorkSchedules'}{/capture}
{$label|strip_semicolon}:
...
title=''               
data-validation="{literal}AEM(callCustomApi(WorkSchedules,canChangeWorkScheduleStatus,{id:$id,status:$status}),'LBL_ERR_CANNOT_CHANGE_WORK_SCHEDULE_STATUS'){/literal}" >
...
{capture name="field_name" assign="field_name"}comments{/capture}
<div class="col-xs-12 col-sm-6 edit-view-row-item" {if $fields.$field_name.acl == 0}style="display: none;"{/if}>
...
{minify}
{capture name="label" assign="label"}{sugar_translate label='LBL_COMMENTS' module='WorkSchedules'}{/capture}
{$label|strip_semicolon}:
...
{if strlen($fields.comments.value) <= 0}
{assign var="value" value=$fields.comments.default_value }
{else}
{assign var="value" value=$fields.comments.value }
{/if}  
<input class="vt_formulaSelector" type='text' name='{$fields.comments.name}' 
id='{$fields.comments.name}' size='30' 
maxlength='255' 
value='{$value}' title=''          
data-dependency="{literal}inArray($type,'occasional_leave','excused_absence'){/literal}"                                         >
</div>
...
{capture name="field_name" assign="field_name"}occasional_leave_type{/capture}
<div class="col-xs-12 col-sm-6 edit-view-row-item" {if $fields.$field_name.acl == 0}style="display: none;"{/if}>
...
{minify}
{capture name="label" assign="label"}{sugar_translate label='LBL_OCCASIONAL_LEAVE_TYPE' module='WorkSchedules'}{/capture}
{$label|strip_semicolon}:
...
{if !isset($config.enable_autocomplete) || $config.enable_autocomplete==false}
<select class="vt_formulaSelector" name="{$fields.occasional_leave_type.name}" 
id="{$fields.occasional_leave_type.name}" 
title=''               
data-dependency="{literal}equals($type,'occasional_leave'){/literal}"                                         >
{if isset($fields.occasional_leave_type.value) && $fields.occasional_leave_type.value != ''}
{html_options options=$fields.occasional_leave_type.options selected=$fields.occasional_leave_type.value}
{else}
{html_options options=$fields.occasional_leave_type.options selected=$fields.occasional_leave_type.default}
{/if}
</select>
{else}
{assign var="field_options" value=$fields.occasional_leave_type.options }
{capture name="field_val"}{$fields.occasional_leave_type.value}{/capture}
{assign var="field_val" value=$smarty.capture.field_val}
{capture name="ac_key"}{$fields.occasional_leave_type.name}{/capture}
{assign var="ac_key" value=$smarty.capture.ac_key}
<select style='display:none' name="{$fields.occasional_leave_type.name}" 
id="{$fields.occasional_leave_type.name}" 
title=''                      
>
{if isset($fields.occasional_leave_type.value) && $fields.occasional_leave_type.value != ''}
...
<input
id="{$fields.occasional_leave_type.name}-input"
name="{$fields.occasional_leave_type.name}-input"
size="30"
value="{$field_val|lookup:$field_options}"
type="text" style="vertical-align: top;">
<span class="id-ff multiple">
<button type="button"><img src="{sugar_getimagepath file="id-ff-down.png"}" id="{$fields.occasional_leave_type.name}-image"></button><button type="button"
id="btn-clear-{$fields.occasional_leave_type.name}-input"
title="Clear"
onclick="SUGAR.clearRelateField(this.form, '{$fields.occasional_leave_type.name}-input', '{$fields.occasional_leave_type.name}'); sync_{$fields.occasional_leave_type.name}()"><span class="suitepicon suitepicon-action-clear"></span></button>
</span>
{literal}
<script>
                    SUGAR.AutoComplete.{/literal}{$ac_key}{literal} = [];
                {/literal}
                                ...
                                {literal}
                    (function (){
                    var selectElem = document.getElementById("{/literal}{$fields.occasional_leave_type.name}{literal}");
                    if (typeof select_defaults == "undefined")
                            select_defaults = [];
                    select_defaults[selectElem.id] = {key:selectElem.value, text:''};
                    //get default
                ...
                {/literal}
                    ...
                    SUGAR.AutoComplete.{$ac_key}.inputNode = Y.one('#{$fields.occasional_leave_type.name}-input');
                    SUGAR.AutoComplete.{$ac_key}.inputImage = Y.one('#{$fields.occasional_leave_type.name}-image');
                    SUGAR.AutoComplete.{$ac_key}.inputHidden = Y.one('#{$fields.occasional_leave_type.name}');
                                {literal}
                    function SyncToHidden(selectme){
                    var selectElem = document.getElementById("{/literal}{$fields.occasional_leave_type.name}{literal}");
                    var doSimulateChange = false;
                    ...
                    //global variable 
                    sync_{/literal}{$fields.occasional_leave_type.name}{literal} = function(){
                    SyncToHidden();
                    }
                    function syncFromHiddenToWidget(){
                    ...
                    if (selectElem.options[i].value == selectElem.value && document.activeElement != document.getElementById('{/literal}{$fields.occasional_leave_type.name}-input{literal}'))
                                SUGAR.AutoComplete.{/literal}{$ac_key}{literal}.inputNode.set('value', selectElem.options[i].innerHTML);
                        }
                        }

                        YAHOO.util.Event.onAvailable("{/literal}{$fields.occasional_leave_type.name}{literal}", syncFromHiddenToWidget);
                {/literal}
...
{minify}
{capture name="label" assign="label"}{sugar_translate label='LBL_DATE_START' module='WorkSchedules'}{/capture}
{$label|strip_semicolon}:
...
<input autocomplete="off" type="text" id="{$fields.date_start.name}_date" value="{$fields[$fields.date_start.name].value}" size="11" maxlength="10" title='' tabindex="0" onblur="combo_{$fields.date_start.name}.update();" onchange="combo_{$fields.date_start.name}.update();  viewTools.form.blur( $( '#{$fields.date_start.name}' ) );"                      class="datetimecombo_date vt_formulaSelector"
>
...
<script type="text/javascript">
    ...
    var combo_{$fields.date_start.name} = new Datetimecombo( "{$fields[$fields.date_start.name].value}", "{$fields.date_start.name}", "{$TIME_FORMAT}", "0", '', false, true, '5' );
...
{minify}
{capture name="label" assign="label"}{sugar_translate label='LBL_DATE_END' module='WorkSchedules'}{/capture}
{$label|strip_semicolon}:
...
<tr valign="middle">
<td nowrap class="dateTimeComboColumn">
<input autocomplete="off" type="text" id="{$fields.date_end.name}_date" value="{$fields[$fields.date_end.name].value}" size="11" maxlength="10" title='' tabindex="0" onblur="combo_{$fields.date_end.name}.update();" onchange="combo_{$fields.date_end.name}.update();  viewTools.form.blur( $( '#{$fields.date_end.name}' ) );"                      class="datetimecombo_date vt_formulaSelector"
>
...
<script type="text/javascript">
    ...
    var combo_{$fields.date_end.name} = new Datetimecombo( "{$fields[$fields.date_end.name].value}", "{$fields.date_end.name}", "{$TIME_FORMAT}", "0", '', false, true, '5' );
...
{capture name="field_name" assign="field_name"}duration_hours{/capture}
<div class="col-xs-12 col-sm-6 edit-view-row-item" {if $fields.$field_name.acl == 0}style="display: none;"{/if}>
...
{minify}
{capture name="label" assign="label"}{sugar_translate label='LBL_DURATION' module='WorkSchedules'}{/capture}
{$label|strip_semicolon}:
...
{counter name="panelFieldCount"  print=false}
{include file="modules/WorkSchedules/tpls/DurationFieldEditView.tpl"}
</div>
...
{minify}
{capture name="label" assign="label"}{sugar_translate label='LBL_DELEGATIONS' module='WorkSchedules'}{/capture}
{$label|strip_semicolon}:
...
data-dependency="{literal}or(equals($type,'delegation'),equals($type,'home')){/literal}"                                         >
...
{capture name="field_name" assign="field_name"}workplace_name{/capture}
<div class="col-xs-12 col-sm-6 edit-view-row-item" {if $fields.$field_name.acl == 0}style="display: none;"{/if}>
...
{minify}
{capture name="label" assign="label"}{sugar_translate label='LBL_RELATIONSHIP_WORKPLACE_NAME' module='WorkSchedules'}{/capture}
{$label|strip_semicolon}:
...
{capture name=idname assign=idname}{$fields.workplace_name.name}{/capture}
<input type="text" name="{$fields.workplace_name.name}" class="vt_formulaSelector sqsEnabled" tabindex="0" id="{$fields.workplace_name.name}" size="" value="{$fields.workplace_name.value}" title='' autocomplete="off"  {if $idname == $locked_field}readonly{/if} 	 
data-dependency="{literal}equals($type,'office'){/literal}"                                          data-validation="{literal}AEM(callCustomApi(WorkSchedules,validateWorkplaceStatus,{id:$workplace_id}),'LBL_ERR_STATUS_NOT_ACTIVE');AEM(callCustomApi(WorkSchedules,validateWorkplaceAllocationPeriods,{workplace_id:$workplace_id,date_start:$date_start,date_end:$date_end}),'LBL_ERR_WORKPLACE_NOT_ACTIVE'){/literal}" >
<input class="vt_formulaSelector" type="hidden" name="{$fields.workplace_name.id_name}" 
id="{$fields.workplace_name.id_name}" 
value="{$fields.workplace_id.value}">
{if $idname != $locked_field}
<span class="id-ff multiple">
<button type="button" name="btn_{$fields.workplace_name.name}" id="btn_{$fields.workplace_name.name}" tabindex="0" title="{sugar_translate label="LBL_SELECT_BUTTON_TITLE"}" class="button firstChild" value="{sugar_translate label="LBL_SELECT_BUTTON_LABEL"}"
onclick='open_popup(
"{$fields.workplace_name.module}",
600,
400,
"&for_employee_id="+this.form.{$fields.assigned_user_id.name}.value+"&availability_advanced=active",
true,
false,
{literal}{"call_back_function":"viewTools.form.function.set_return","form_name":"EditView","field_to_name_array":{"id":"workplace_id","name":"workplace_name"}}{/literal},
"single",
true
);' ><span class="suitepicon suitepicon-action-select"></span></button><button type="button" name="btn_clr_{$fields.workplace_name.name}" id="btn_clr_{$fields.workplace_name.name}" tabindex="0" title="{sugar_translate label="LBL_ACCESSKEY_CLEAR_RELATE_TITLE"}"  class="button lastChild"
onclick="SUGAR.clearRelateField( this.form, '{$fields.workplace_name.name}', '{$fields.workplace_name.id_name}' ); $( '#{$fields.workplace_name.name},#{$fields.workplace_name.id_name}' ).blur();"  value="{sugar_translate label="LBL_ACCESSKEY_CLEAR_RELATE_LABEL"}" ><span class="suitepicon suitepicon-action-clear"></span></button>
</span>
{/if}
<script type="text/javascript">
        SUGAR.util.doWhen(
                "typeof(sqs_objects) != 'undefined' && typeof(sqs_objects['{$form_name}_{$fields.workplace_name.name}']) != 'undefined'",
                enableQS
                );
    </script>
</div>
...
{capture name="field_name" assign="field_name"}delegation_duration{/capture}
<div class="col-xs-12 col-sm-12 edit-view-row-item" {if $fields.$field_name.acl == 0}style="display: none;"{/if}>
...
{minify}
{capture name="label" assign="label"}{sugar_translate label='LBL_DELEGATION_DURATION' module='WorkSchedules'}{/capture}
{$label|strip_semicolon}:
...
{if strlen($fields.delegation_duration.value) <= 0}
{assign var="value" value=$fields.delegation_duration.default_value }
{else}
{assign var="value" value=$fields.delegation_duration.value }
{/if}  
<input class="vt_formulaSelector" type='text' name='{$fields.delegation_duration.name}'
id='{$fields.delegation_duration.name}'
size='30'
maxlength='18'       value='{sugar_number_format var=$value precision=2 }'
title=''
tabindex='0'
data-dependency="{literal}equals($type,'delegation'){/literal}"                                          data-validation="{literal}AEM(ifElse(equals($type,'delegation'),callCustomApi(WorkSchedules,validateDelegationDurationValue,$delegation_duration),true),'LBL_ERR_DELEGATION_DURATION_NOT_VALID'){/literal}"  >
</div>
...
{minify}
{capture name="label" assign="label"}{sugar_translate label='LBL_DESCRIPTION' module='WorkSchedules'}{/capture}
{$label|strip_semicolon}:
...
<a class="" role="button" data-toggle="collapse-edit" aria-expanded="false">
<div class="col-xs-10 col-sm-11 col-md-11">
{sugar_translate label='LBL_REPEAT_TAB' module='WorkSchedules'}
</div>
</a>
</div>
...
{capture name="field_name" assign="field_name"}repeat_pane{/capture}
<div class="col-xs-12 col-sm-12 edit-view-row-item" {if $fields.$field_name.acl == 0}style="display: none;"{/if}>
...
{counter name="panelFieldCount"  print=false}
{include file="modules/WorkSchedules/tpls/RepeatPanelEditView.tpl"}
</div>
...
{/literal}{literal}
<script type="text/javascript">
addForm('EditView');addToValidate('EditView', 'name', 'name', true,'{/literal}{sugar_translate label='LBL_NAME' module='WorkSchedules' for_js=true}{literal}' );
addToValidate('EditView', 'date_entered_date', 'date', false,'Date Created' );
addToValidate('EditView', 'date_modified_date', 'date', false,'Date Modified' );
addToValidate('EditView', 'modified_user_id', 'assigned_user_name', false,'{/literal}{sugar_translate label='LBL_MODIFIED' module='WorkSchedules' for_js=true}{literal}' );
addToValidate('EditView', 'modified_by_name', 'relate', false,'{/literal}{sugar_translate label='LBL_MODIFIED_NAME' module='WorkSchedules' for_js=true}{literal}' );
addToValidate('EditView', 'created_by', 'assigned_user_name', false,'{/literal}{sugar_translate label='LBL_CREATED' module='WorkSchedules' for_js=true}{literal}' );
addToValidate('EditView', 'created_by_name', 'relate', false,'{/literal}{sugar_translate label='LBL_CREATED' module='WorkSchedules' for_js=true}{literal}' );
addToValidate('EditView', 'description', 'text', false,'{/literal}{sugar_translate label='LBL_DESCRIPTION' module='WorkSchedules' for_js=true}{literal}' );
addToValidate('EditView', 'deleted', 'bool', false,'{/literal}{sugar_translate label='LBL_DELETED' module='WorkSchedules' for_js=true}{literal}' );
addToValidate('EditView', 'assigned_user_id', 'relate', false,'{/literal}{sugar_translate label='LBL_ASSIGNED_TO_ID' module='WorkSchedules' for_js=true}{literal}' );
addToValidate('EditView', 'assigned_user_name', 'relate', false,'{/literal}{sugar_translate label='LBL_ASSIGNED_TO_NAME' module='WorkSchedules' for_js=true}{literal}' );
addToValidate('EditView', 'repeat_pane', 'varchar', false,'{/literal}{sugar_translate label='LBL_REPEAT_PANE' module='WorkSchedules' for_js=true}{literal}' );
addToValidate('EditView', 'repeat_type', 'enum', false,'{/literal}{sugar_translate label='LBL_REPEAT_TYPE' module='WorkSchedules' for_js=true}{literal}' );
addToValidate('EditView', 'repeat_interval', 'int', false,'{/literal}{sugar_translate label='LBL_REPEAT_INTERVAL' module='WorkSchedules' for_js=true}{literal}' );
addToValidate('EditView', 'repeat_dow', 'varchar', false,'{/literal}{sugar_translate label='LBL_REPEAT_DOW' module='WorkSchedules' for_js=true}{literal}' );
addToValidate('EditView', 'repeat_until', 'date', false,'{/literal}{sugar_translate label='LBL_REPEAT_UNTIL' module='WorkSchedules' for_js=true}{literal}' );
addToValidate('EditView', 'repeat_count', 'int', false,'{/literal}{sugar_translate label='LBL_REPEAT_COUNT' module='WorkSchedules' for_js=true}{literal}' );
addToValidate('EditView', 'repeat_parent_id', 'id', false,'{/literal}{sugar_translate label='LBL_REPEAT_PARENT_ID' module='WorkSchedules' for_js=true}{literal}' );
addToValidate('EditView', 'lp', 'int', false,'{/literal}{sugar_translate label='LBL_LP' module='WorkSchedules' for_js=true}{literal}' );
addToValidate('EditView', 'type', 'enum', false,'{/literal}{sugar_translate label='LBL_TYPE' module='WorkSchedules' for_js=true}{literal}' );
addToValidate('EditView', 'status', 'enum', false,'{/literal}{sugar_translate label='LBL_STATUS' module='WorkSchedules' for_js=true}{literal}' );
addToValidate('EditView', 'supervisor_acceptance', 'enum', false,'{/literal}{sugar_translate label='LBL_SUPERVISOR_ACCEPTANCE' module='WorkSchedules' for_js=true}{literal}' );
addToValidate('EditView', 'schedule_date', 'date', false,'{/literal}{sugar_translate label='LBL_SCHEDULE_DATE' module='WorkSchedules' for_js=true}{literal}' );
addToValidate('EditView', 'date_start_date', 'date', true,'Start Time' );
addToValidate('EditView', 'date_end_date', 'date', true,'End Time' );
addToValidate('EditView', 'duration_minutes', 'int', false,'{/literal}{sugar_translate label='LBL_DURATION_MINUTES' module='WorkSchedules' for_js=true}{literal}' );
addToValidate('EditView', 'duration_hours', 'int', true,'{/literal}{sugar_translate label='LBL_DURATION_HOURS' module='WorkSchedules' for_js=true}{literal}' );
addToValidate('EditView', 'spent_time', 'float', false,'{/literal}{sugar_translate label='LBL_SPENT_TIME' module='WorkSchedules' for_js=true}{literal}' );
addToValidate('EditView', 'spent_time_settlement', 'float', false,'{/literal}{sugar_translate label='LBL_SPENT_TIME_SETTLEMENT' module='WorkSchedules' for_js=true}{literal}' );
addToValidate('EditView', 'delegation_duration', 'float', true,'{/literal}{sugar_translate label='LBL_DELEGATION_DURATION' module='WorkSchedules' for_js=true}{literal}' );
addToValidate('EditView', 'comments', 'varchar', false,'{/literal}{sugar_translate label='LBL_COMMENTS' module='WorkSchedules' for_js=true}{literal}' );
addToValidate('EditView', 'time_tracking_pane', 'varchar', false,'{/literal}{sugar_translate label='LBL_TIME_TRACKING_PANE' module='WorkSchedules' for_js=true}{literal}' );
addToValidate('EditView', 'occasional_leave_type', 'enum', true,'{/literal}{sugar_translate label='LBL_OCCASIONAL_LEAVE_TYPE' module='WorkSchedules' for_js=true}{literal}' );
addToValidate('EditView', 'delegation_name', 'relate', false,'{/literal}{sugar_translate label='LBL_DELEGATIONS' module='WorkSchedules' for_js=true}{literal}' );
addToValidate('EditView', 'delegation_id', 'id', false,'{/literal}{sugar_translate label='LBL_DELEGATIONS' module='WorkSchedules' for_js=true}{literal}' );
addToValidate('EditView', 'workplace_name', 'relate', false,'{/literal}{sugar_translate label='LBL_RELATIONSHIP_WORKPLACE_NAME' module='WorkSchedules' for_js=true}{literal}' );
addToValidate('EditView', 'workplace_id', 'id', false,'{/literal}{sugar_translate label='LBL_RELATIONSHIP_WORKPLACE_ID' module='WorkSchedules' for_js=true}{literal}' );
addToValidateBinaryDependency('EditView', 'assigned_user_name', 'alpha', false,'{/literal}{sugar_translate label='ERR_SQS_NO_MATCH_FIELD' module='WorkSchedules' for_js=true}{literal}: {/literal}{sugar_translate label='LBL_ASSIGNED_TO' module='WorkSchedules' for_js=true}{literal}', 'assigned_user_id' );
addToValidateBinaryDependency('EditView', 'delegation_name', 'alpha', false,'{/literal}{sugar_translate label='ERR_SQS_NO_MATCH_FIELD' module='WorkSchedules' for_js=true}{literal}: {/literal}{sugar_translate label='LBL_DELEGATIONS' module='WorkSchedules' for_js=true}{literal}', 'delegation_id' );
addToValidateBinaryDependency('EditView', 'workplace_name', 'alpha', false,'{/literal}{sugar_translate label='ERR_SQS_NO_MATCH_FIELD' module='WorkSchedules' for_js=true}{literal}: {/literal}{sugar_translate label='LBL_RELATIONSHIP_WORKPLACE_NAME' module='WorkSchedules' for_js=true}{literal}', 'workplace_id' );
